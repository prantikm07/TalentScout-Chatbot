...
def generate_pdf_report():
    ...
    for idx, candidate in enumerate(st.session_state.all_candidates):
        ...
        basic_data = [
            ...
            ["Rating", str(st.session_state.candidate_ratings.get(candidate.get("email", ""), "Not rated"))]
        ]
    ...
    with st.sidebar:
        ...
        if st.session_state.admin_logged_in:
            ...
            if st.session_state.all_candidates:
                ...
                # Rate the candidate
                st.subheader("Rate Candidate")
                current_rating = st.session_state.candidate_ratings.get(selected_candidate_email, 5)
                new_rating = st.slider("Rating (0-10)", 0, 10, int(current_rating))
                
                # Display color code based on rating
                if new_rating >= 8:
                    st.success("High potential candidate")
                elif new_rating >= 4:
                    st.warning("Average potential candidate")
                else:
                    st.error("Low potential candidate")
                
                # Save the rating
                if st.button("Save Rating"):
                    st.session_state.candidate_ratings[selected_candidate_email] = new_rating
